...
void ICP::readfile(string firstname,string secondname)
{
	...
	STLloader loader(firstname);
	loader.load();////first model loading
	vector <GLfloat> pointsP = loader.get_vertexs();
	int vertex_count = int(pointsP.size()/3);
	for (int i = 0; i<vertex_count;i++){
		Vertex v;
		v.coord[0] = double(pointsP.at(i * 3 + 0));
		v.coord[1] = double(pointsP.at(i * 3 + 1));
		v.coord[2] = double(pointsP.at(i * 3 + 2));
		VarrP.push_back(v);
	}
	STLloader loader2(secondname);
	loader2.set_path(secondname);
	loader2.load();////second model loading
	vector <GLfloat> pointsQ = loader2.get_vertexs();
	vertex_count = int(pointsQ.size() / 3);
	for (int i = 0; i<vertex_count;i++){
		Vertex v;
		v.coord[0] = double(pointsQ.at(i * 3 + 0));
		v.coord[1] = double(pointsQ.at(i * 3 + 1));
		v.coord[2] = double(pointsQ.at(i * 3 + 2));
		VarrQ.push_back(v);
	}
	cout << endl;
...
void ICP::transform()
{
	...
	Matx44d U;
	Matx44d V;
	//cv::Mat V(4, 4, CV_32F, 0.0f);
	Mat D = Mat::eye(4, 4, CV_64F);
	/*
	Mat C = Mat::zeros(4, 4, CV_32F);
	Mat D = C.diag(0);
	D.at<unsigned char>(0) = 32;
	D.at<unsigned char>(1) = 32;
	D.at<unsigned char>(2) = 32;
	D.at<unsigned char>(3) = 32;
	*/
	SVD::compute(B, D, U, V,cv::SVD::FULL_UV);
	//
